    ...
    <header className="fixed top-0 left-0 right-0 z-50 flex justify-center px-4 sm:px-6 pt-4 pointer-events-none">
      <nav
        className={`pointer-events-auto w-full max-w-5xl rounded-full transition-all duration-300 flex items-center justify-between px-4 sm:px-6 py-2.5 ${
          scrolled
            ? "bg-[#06080E]/85 backdrop-blur-xl border border-white/[0.12] shadow-[0_12px_40px_rgba(0,0,0,0.6)]"
            : "bg-[#070A12]/60 backdrop-blur-lg border border-white/[0.08] shadow-[0_8px_32px_rgba(0,0,0,0.35)]"
        }`}
      >
        {/* Logo with Glowing DNA wave */}
        <Link href="/" className="flex items-center gap-2.5 group">
          <div className="relative h-8 w-8 rounded-full bg-gradient-to-tr from-cyan-500 via-blue-600 to-indigo-600 p-[1.5px] shadow-[0_0_12px_rgba(0,229,255,0.35)] group-hover:shadow-[0_0_20px_rgba(0,229,255,0.6)] transition-shadow">
            <div className="h-full w-full rounded-full bg-[#080B11] flex items-center justify-center">
              <Shield className="h-4 w-4 text-cyan-400 group-hover:scale-105 transition-transform" />
            </div>
          </div>
          <div className="flex flex-col">
            <span className="text-sm font-semibold tracking-tight text-white flex items-center gap-1">
              Fraud<span className="bg-gradient-to-r from-cyan-400 to-blue-400 bg-clip-text text-transparent font-bold">DNA</span>
            </span>
        ...
        {/* Center Nav Links */}
        <div className="hidden md:flex items-center gap-1 lg:gap-1.5 px-3 py-1 rounded-full bg-white/[0.02] border border-white/[0.05]">
          {navLinks.map((link) => {
            const isActive =
              link.activePath === "/"
                ? pathname === "/"
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <button
            onClick={toggleTheme}
            className="p-2 rounded-full text-slate-400 hover:text-white hover:bg-white/[0.08] transition-colors border border-white/[0.05] cursor-pointer"
      ...
      {mobileMenuOpen && (
        <div className="pointer-events-auto absolute top-18 inset-x-4 max-w-lg mx-auto bg-[#070A12]/95 backdrop-blur-2xl rounded-2xl border border-white/[0.12] p-4 shadow-2xl space-y-2 md:hidden">
          <div className="flex flex-col space-y-1">
            {navLinks.map((link) => (
              <Link
                ...
                className="px-4 py-2.5 rounded-xl text-sm font-medium text-slate-300 hover:text-white hover:bg-white/[0.06] transition-colors flex items-center justify-between"
              >
